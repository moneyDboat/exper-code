...
class DynamicMemoryCell(tf.contrib.rnn.RNNCell):
    ...
    def get_gate(self, state_j, key_j, inputs, v=None, prev_a=None):
        ...
        g_j <- \sigma(s_t^T h_j + s_t^T w_j + v^T d_j)
        """
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__, initializer=self._initializer):
            U = tf.get_variable('U', [self._num_units_per_block, self._num_units_per_block],
                                initializer=self._recurrent_initializer)  # 300*300
            ...
            state, state_a = tf.split(
                value=state,
                num_or_size_splits=[
                    self._num_blocks * self._num_units_per_block,  # (7, 300)
                    self._num_blocks * self._num_units_per_block
                ],
                axis=1,
            )
            state_a = tf.split(state_a, self._num_blocks, axis=1)  # (7, 300)
            assert len(state_a) == self._num_blocks

            # Split the hidden state into blocks (each U, V, W are shared across blocks).
            state = tf.split(state, self._num_blocks, axis=1)  # (7, 300)
            ...
            for j, state_j in enumerate(state):  # Hidden State (j)
                key_j = tf.expand_dims(self._keys[j], axis=0)  # 扩展一维  1*300
                ...
                with tf.variable_scope("entnet_gru", reuse=reuse) as gru_scope:
                    ...
                    # 得到GRU的hidden state
                    # Equation 4: d_ij = GRU(h_j^~, d_i-1j)
...
class Delayed_EntNet_Sentihood(object):
    ...
    def _build_inputs(self):  # 设定占位符，暂时存储变量
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):  # 实现参数共享的功能
    ...
    def _mask_embedding(self, embedding):
        vocab_size, embedding_size = self._embedding_mat.shape  # 40002， 300
    ...
    def _inference_adj(self, sentences, targets, aspects,
        ...
        with tf.variable_scope(self._name):
            ...
            batch_size = tf.shape(sentences)[0]  # 128

            targets_emb = tf.nn.embedding_lookup(masked_embedding, targets)  # (?, 1, 300)
            ...
            tied_keys_emb = tf.nn.embedding_lookup(masked_embedding, self._tied_keys)  # (2, 300)
            tied_keys_emb = tf.reduce_mean(
                input_tensor=tied_keys_emb,
                axis=1,
            )
            # [len(self._tied_keys), emb_size]
            free_keys_emb = self._free_keys_embedding  # 值随机初始化(5, 300)
            ...
            # final classifier
            asp_att = tf.concat(values=[targets_emb, aspects_emb], axis=2)  # (1, 600)
            # [None, 1, emb_size * 2]
