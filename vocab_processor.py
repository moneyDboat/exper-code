...
class EmbeddingVocabulary(tf.contrib.learn.preprocessing.CategoricalVocabulary):
    ...
    def _load_embeddings(self, in_file, binary=False):
        ...
        with open(in_file) as in_f:
            ...
            self._embeddings = np.zeros((400002, 300), dtype=np.float32)
            for line in in_f:
                line = line.strip()
                attrs = line.split(' ')
                # 所用的glove第一行并没有词汇量和维度信息
                # if len(attrs) == 2:
            ...
            self._embeddings[self._mapping[self._unknown_token]] = unk  # 用平均值作为unknown_token的embedding
    ...
    def get(self, category):
        ...
        """
        if category not in self._mapping:
            if self._freeze:  # 如果为freeze则所有没命中的词都归为unknown_token
                return self._mapping[self._unknown_token]
