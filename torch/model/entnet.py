...
class EntNet(nn.Module):
    def __init__(self, config, vectors=None, target_vectors=None, aspect_vectors=None):
        ...
        tired_keys = [1, 2]  # ['Location1', 'Location2']
        tired_keys_embed = self.embedding(tired_keys)  # 2 * 300
        # 5*300，训练过程中会不断更新
        free_keys_embed = nn.Parameter(torch.Tensor(self.config.n_keys - self.config.n_tied), self.config.embedding_dim)
        keys_emb = torch.cat([tired_keys_embed, free_keys_embed], dim=0)  # 7* 300
        self.keys = keys_emb

        # 前向和后向
        self.ent_cell_fw = EntNetCell(config.n_keys, config.embedding_dim, keys_emb)
        self.ent_cell_bw = EntNetCell(config.n_keys, config.embedding_dim, keys_emb)

        # final classifier
        self.W_att = torch.randn(config.embedding_dim, config.embedding_dim * 2)  # 300*600
    ...
    def forward(self, text, target, aspect):
        ...
        # 参数初始化
        h_fw = torch.randn(batch_size, self.config.embedding_dim)
        b_fw = torch.randn(batch_size, self.config.embedding_dim)
        ...
        for text in text_embed:
            h_fw, b_fw = self.ent_cell_fw(h_fw, b_bw)
            # 双向后面再改
            # h_bw, b_bw = self.ent_cell_bw(h_bw, b_bw)

        # p_j = softmax(k_j W_att [t a])
        last_h_fw = h_fw  # 7 * 128 * 300
        target_aspect = torch.cat([target_embed, aspect_embed], dim=1)  # 128 * 600
        u = torch.zeros(batch_size, self.config.embedding_dim)  # 128 * 300
        for h_j, key in zip(last_h_fw, self.keys):
            att = torch.mm(key.unsqueeze(dim=0), self.W_att)
            att = torch.mm(att, target_aspect.permute(1, 0))
            att = F.softmax(att).permute(1, 0)  # 128 * 1
            u = u + att * h_j
...
class EntNetCell(nn.Module):
    def __init__(self, num_blocks, num_unit_per_block, keys):
        ...
        self.U = nn.Parameter(torch.Tensor(self.num_unit_per_block, self.num_unit_per_block))
        self.V = nn.Parameter(torch.Tensor(self.num_unit_per_block, self.num_unit_per_block))
        self.W = nn.Parameter(torch.Tensor(self.num_unit_per_block, self.num_unit_per_block))
        self.bias = nn.Parameter(torch.Tensor(self.num_unit_per_block))
        self.v = nn.Parameter(torch.Tensor(self.num_unit_per_block))
    ...
    def forward(self, w, h, d):
        ...
        for j, h_j in enumerate(h):
            ...
            new_d.append(self.gru_cell(candi_j, d[j]))

            gate_j = self.get_gate(h_j, k_j, w, new_d)
            ...
            h_j_next_norm = torch.norm(h_j_next, dim=0)
    ...
    def get_gate(self, h_j, k_j, w, d_j):
        ...
        c = torch.mm(d_j, self.v.unsqueeze(dim=1))  # batch
        return nn.Sigmoid(a + b + c)
